...
then
  ...
  # Create database
  mysql -u root --password=$ROOT_DB_PASS -e "drop database if exists engineblock; create database engineblock default charset utf8 default collate utf8_unicode_ci;"
  cat $OC_BASEDIR/data/engineblock.sql | \
    sed -e "s/_OPENCONEXT_DOMAIN_/$OC_DOMAIN/g" | \
    mysql -u root --password=$ROOT_DB_PASS engineblock

  # Set database creadentials for engine
  mysql -uroot -p$ROOT_DB_PASS -e "GRANT ALL PRIVILEGES ON engineblock.* TO $ENGINE_DB_USER@localhost IDENTIFIED BY '$ENGINE_DB_PASS'"
  ...
  sed -i "s/\[engineblock_db_user\]/$ENGINE_DB_USER/g" /etc/surfconext/engineblock.ini
  sed -i "s/\[engineblock_db_pass\]/$ENGINE_DB_PASS/g" /etc/surfconext/engineblock.ini

  # Apply janus api credentials to file engineblock.ini
  sed -i "s/\[engine_janusapi_user\]/$ENGINE_JANUSAPI_USER/g" /etc/surfconext/engineblock.ini
  sed -i "s/\[engine_janusapi_pass\]/$ENGINE_JANUSAPI_PASS/g" /etc/surfconext/engineblock.ini
  ...
  ldapmodify -x -D cn=admin,cn=config -h localhost -w $LDAP_PASS -f /opt/www/engineblock/ldap/changes/addDeprovisionWarningSentAttributes.ldif
  ldapmodify -x -D cn=admin,cn=config -h localhost -w $LDAP_PASS -f /opt/www/engineblock/ldap/changes/addCollabPersonUUID.ldif

  # Apply LDAP credentials to file engineblock.ini
  sed -i "s/\[engine_ldap_passwd\]/$LDAP_PASS/g" /etc/surfconext/engineblock.ini
